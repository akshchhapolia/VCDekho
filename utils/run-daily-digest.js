/**
 * Shared daily digest publisher.
 * Used by api/cron/daily-digest.js and chained from api/cron/scrape.js after ai-process.
 */
const db = require('./db');
const { generateText, stripCodeFences, PROSE_MODEL } = require('./gemini');
const {
  formatDigestLabel,
...
async function runDailyDigest(options = {}) {
  ...
  if (pendingItems.length === 0) {
    return {
      message: 'No items pending for digest.',
      itemsProcessed: 0,
