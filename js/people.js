(function (global) {
    ...
    els.results.innerHTML = people.map((p) => {
      ...
      const emailHtml = p.email
        ? (window.VCPersonEmailUnlock
          ...
          : esc(p.email))
        : p.hasEmail
          ? '<a class="inv-email-unlock-btn" href="/login#/investors/' + esc(p.slug) + '" data-unlock-email data-person-slug="' + esc(p.slug) + '">' + LOCK_ICON + '<span class="inv-email-unlock-label">Unlock email</span></a>'
  ...
  async function load(opts) {
    ...
    const res = await directoryFetch(`/api/people?${params.toString()}`);
    if (!res) return;
      ...
      els.results.innerHTML =
        '<div class="inv-dir-empty-state">' +
          '<p class="inv-dir-empty-title">' +
            (err && err.message === 'rate_limit' ? 'Too many requests' : 'Couldn’t load investors') +
          '</p>' +
