...
function writeDirectoryIndexFiles() {
  const fs = require('fs');
  const path = require('path');
  const dir = path.join(process.cwd(), 'public', 'dir-index');
  fs.mkdirSync(dir, { recursive: true });
  const people = getPeopleDirectoryIndex();
  fs.writeFileSync(path.join(dir, 'people.json'), JSON.stringify({ people: people.people }));
  const funds = getFundsDirectoryIndex();
  fs.writeFileSync(path.join(dir, 'funds.json'), JSON.stringify({ investors: funds.investors }));
}

function ensureDirectoryIndexFiles() {
  const fs = require('fs');
  const path = require('path');
  const file = path.join(process.cwd(), 'public', 'dir-index', 'people.json');
  if (!fs.existsSync(file)) writeDirectoryIndexFiles();
}
